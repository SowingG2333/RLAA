...
# --- 日志设置 (将在 main 函数中配置) ---
# 删除了顶层的 logging.basicConfig

def load_jsonl(filepath: str) -> List[Dict[str, Any]]:
    """(与 FgAA_local.py 逻辑相同)"""
    data = []
    try:
...
def parse_attacker_output(response_text: str) -> Dict[str, Any]:
    """从攻击者的自由格式输出中解析推理和猜测(JSON) (来自 FgAA_local.py)"""
    inference = ""
    guess_json = {}
...
def compare_profiles(true_profile: Dict[str, Any], guessed_profile: Dict[str, Any]) -> List[str]:
    """使用确定性的 Python 逻辑比较 profiles (来自 FgAA_local.py)"""
...
# --- FgAA Core Logic (与 FgAA_local.py 逻辑一致) ---
